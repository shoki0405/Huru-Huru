...
if (!empty($_POST["submit"])) {
    ...
    if (!empty($_FILES["picture"]["name"])) {
        if (!move_uploaded_file($_FILES["picture"]['tmp_name'], TMP . $_FILES["picture"]['name'])) {
            $error_message = array_merge(
                $error_message,
                [
                    "画像を送信できませんでした。ご確認の上もう一度お試しください。",
                ]
            );
        }
    } else {
    ...
    if ($error_message == []) {
        if (send(ENTRY, $data, $_FILES["picture"])) {
            $success_message = "応募を受け付けました。<br>2,3営業日以内に入力いただいたメールアドレスにご連絡させていただきますので、少々お待ちください。";
            $_POST = [];
        } else {
            $error_message = array_merge(
                $error_message,
                [
                    "送信できませんでした。<br>入力内容をご確認いただくか、しばらくお待ちいただいた後再度お問い合わせください。",
                ]
            );
        }
    }
    unlink(TMP . $_FILES["picture"]['name']);
}
